...
async function scrapeTorrents(movieTitle) {
    ...
    try {
        ...
        torrentRows.each((i, element) => {
            ...
            const leechText = $(element).text().match(/Stahuju : (\d+)/); // Získání počtu leechů
            const leechers = leechText ? parseInt(leechText[1]) : 0;

            if (detailLink && seeds > 0 && leechers > 0) { // Filtrování: seedy > 0 a leechе > 0
                const infoHashMatch = detailLink.match(/id=([a-fA-F0-9]{40})/i);
                if (infoHashMatch) {
                    const infoHash = infoHashMatch[1].toLowerCase();
                    console.log("Nalezen torrent:", { title, infoHash, category, seeds, leechers });
