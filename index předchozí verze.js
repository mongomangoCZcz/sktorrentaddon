...
const manifest = {
    ...
    version: "1.0.5", // Zvýšena verze kvůli změnám
...
async function scrapeTorrents(movieTitle) {
    ...
    try {
        ...
        const streams = [];
        for (let i = 0; i < torrentRows.length; i++) { // Používáme for kvůli await
            ...
            if (detailLink && seeds > 0 && leechers > 0) {
                const infoHashMatch = detailLink.match(/id=([a-fA-F0-9]{40})/i);
                if (infoHashMatch) {
                    const infoHash = infoHashMatch[1].toLowerCase();
                    // Získání detailní stránky pro odkaz na download.php
                    ...
                    if (downloadLink) {
                        console.log("Nalezen torrent:", { title, infoHash, category, seeds, leechers, downloadLink });
                        if (category.includes("Filmy") || category.includes("Seriál") || category.includes("TV Pořad")) {
                            streams.push({
                                name: `SKTorrent: ${title} (${seeds} seedů, ${leechers} leechů)`,
                                infoHash: infoHash,
                                sources: [`torrent:magnet:?xt=urn:btih:${infoHash}&tr=udp://tracker.opentrackr.org:1337/announce`],
                                url: `https://sktorrent.eu/torrent/${downloadLink}` // Přidán odkaz na .torrent soubor
                            });
                        }
                    } else {
                        console.log(`Nepodařilo se najít download link pro: ${title}`);
                    }
                }
            } else {
                console.log(`Torrent přeskočen (nedostatek seedů/leechů): ${title} (seedy: ${seeds}, leechе: ${leechers})`);
            }
        }
